...
import {
  ...
  type RunPackageCommand,
} from "../../src/cli/projectInstall.js";
...
describe("ensureFreshSqueezyDevDependency", () => {
  ...
  it("uses npm by default and installs the current package version", async () => {
    ...
    expect(runCommand).toHaveBeenCalledWith("npm", ["install", "--save-dev", "fresh-squeezy@0.1.8"], {
      cwd: workdir,
    });
  });
  ...
  it("surfaces a failed package manager command", async () => {
    ...
    await expect(
      ensureFreshSqueezyDevDependency({ cwd: workdir, runCommand })
    ).rejects.toThrow("npm install --save-dev fresh-squeezy failed with exit code 1");
  });
});
