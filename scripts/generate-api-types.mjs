 ...
 *
 * Usage:
 *   node scripts/generate-api-types.mjs            # check generated file
 *   node scripts/generate-api-types.mjs --update   # refresh generated file
 */
...
async function fetchText(url) {
  const response = await fetch(url, {
    headers: {
      "user-agent": "fresh-squeezy-api-type-generator/0.1 (+https://github.com/YosefHayim/fresh-squeezy)",
      accept: "text/html,*/*",
    },
  });
...
function renderTypes(resources) {
  ...
  lines.push("}", "");
  lines.push("export type GeneratedLemonSqueezyResourceName = keyof GeneratedLemonSqueezyFieldMap;");
  lines.push("export type GeneratedLemonSqueezyResourceType = (typeof GENERATED_LEMON_SQUEEZY_RESOURCES)[number];");
  lines.push("");
...
function nullFallbackType(key) {
  ...
  if (/(^|_)(url|email|name|status|currency|country|region|city|interval|reason|brand|date|at)(_|$)/.test(key)) {
    return "string";
  }
  return "unknown";
}
